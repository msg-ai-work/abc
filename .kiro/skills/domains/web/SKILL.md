...
6. 변경 작업이면 사용자 영향, backward compatibility, rollback을 확인한다.
...
- [ ] 요구사항과 화면/API acceptance criteria 매핑
- [ ] 인증/인가 누락 여부 확인
- [ ] 입력값 검증과 오류 메시지 확인
- [ ] API schema 호환성 확인
- [ ] 개인정보 노출 및 로그 마스킹 확인
...
- 운영 사용자 개인정보나 토큰 원문을 Git에 저장하지 않는다.
- AI가 운영 권한 변경, 사용자 잠금해제, DB 데이터 수정, Production 배포를 직접 실행하지 않는다.
